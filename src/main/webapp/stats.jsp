...
<body>
  <%
  VoteDataService dataSvc = new VoteDataService();
  ...
  <ul id="total">
    ...
    <li class="indi">
      <ul>
        <li class="info"><a href="indi.jsp?idx=<%=each.getStats().getCandiIdx()%>&name='<%=each.getStats().getCandiName()%>'"><%=each.getStats().getCandiIdx()%>번 <%=each.getStats().getCandiName()%></a>
        </li>
        <li class="rate">
